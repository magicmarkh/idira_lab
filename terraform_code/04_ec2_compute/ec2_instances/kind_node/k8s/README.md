# swa-probe — Idira Secure Workload Access connectivity test
...
## Files
- `main.go` — Go probe (go-spiffe; static binary, fits scratch/distroless)
...
2. Requests a JWT-SVID for `SWA_AUDIENCE` (default `conjur`) from the node-local
   SWA Agent via the **SPIFFE Workload API** (gRPC-over-UDS, go-spiffe).
3. Decodes the JWT payload (unverified, display only) and prints `sub`
   (your SPIFFE ID), `aud`, `iss`, `exp`. Exit 0 on success.

## Deploy

**Automated (recommended).** Enable the SWA layer in Terraform — it installs the
agent, builds + loads this image, and deploys + verifies the probe:
- Set the variables in [`../../swa.tfvars.example`](../../swa.tfvars.example)
  (a fragment to append to your `04_ec2_compute` `terraform.tfvars`), then
  `terraform apply`. The Ansible role `swa_workloads` does the work.
- Then run the presenter demo scripts in `scripts/demo/swa/`.

**Manual (for reference).** On the node:
```bash
go mod tidy                       # generate go.sum (needs network)
docker build -t swa-probe:0.1.0 .
kind load docker-image swa-probe:0.1.0 --name kind
kubectl apply -f k8s.yaml
kubectl -n swa-probe logs deploy/swa-probe
```
Success looks like:
```
[swa-probe] SUCCESS: fetched a JWT-SVID (…)
[swa-probe]   sub (SPIFFE ID) : spiffe://<trust-domain>/k8s-nodegroup/ns/swa-probe/sa/swa-probe
[swa-probe]   aud             : conjur
```

## Config (env vars)
The probe speaks the SPIFFE Workload API (gRPC-over-UDS) via go-spiffe, so the
only knobs it reads are the four below (see `swa_probe/main.go`). The earlier
`SWA_METHOD` / `SWA_HTTP_PATH` / `SWA_AGENT_BIN` HTTP/CLI options are gone.

| var | default | notes |
|---|---|---|
| `SWA_SOCKET_PATH` | `/tmp/swa-agent/public/api.sock` | must match the agent's hostPath socket |

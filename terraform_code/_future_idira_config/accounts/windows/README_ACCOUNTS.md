...
This Terraform configuration demonstrates a pattern for managing Windows domain accounts in Idira Privilege Cloud. Each account type is organized into its own safe with corresponding accounts, making it easy to expand and customize for your environment.
...
The following attributes are managed by Idira and ignored by Terraform after initial creation:

| Attribute | Managed By | Notes |
|-----------|------------|-------|
| `secret` | Idira CPM | Automatically rotated |
| `account_id` | Idira | Assigned at creation |
| `created_time` | Idira | Timestamp management |
| `remote_machines` | Idira | v0.1.17+: List of allowed remote machines |
| `access_restricted_to_remote_machines` | Idira | v0.1.17+: Boolean restriction flag |
| `platform_account_properties` | Idira Platform | Platform-specific settings |
| `status` | Idira | Account status |

**Important:** Terraform creates accounts with the specified `secret`, but Idira CPM manages password rotation thereafter. Do not update passwords through Terraform after initial creation.
...
3. Run `terraform apply` to remove from Idira
...
Choose Idira platforms based on account type and operating system:
...
Ensure platforms exist in your Idira environment before creating accounts.

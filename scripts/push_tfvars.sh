...
sanitize_tfvars() {
    ...
    sed -i.bak -E 's/^(conjur_authenticator_name[[:space:]]*=[[:space:]]*).*/\1""/' "${output_file}"
...
main() {
    ...
    echo "Sensitive fields (conjur_login, conjur_api_key, conjur_authenticator_name, conjur_host_id) will be cleared."

...
Terraform configuration to onboard an AWS Organization and all its member accounts into Idira Cloud Entitlements (CCE). This deploys IAM roles across the organization via CloudFormation StackSets and registers each member account with the Idira CCE API.

## Architecture

1. **Organization onboarding** -- The `cyberark/cce-organization/aws` module registers the AWS Organization with Idira and creates IAM roles in the management account for CCE scanning and SCA/SIA services.
...
4. **Account registration** -- Each discovered member account is registered with the Idira CCE API, associating it with the deployed IAM roles.

## Prerequisites

- Terraform >= 1.8.5
- AWS CLI configured with **management account** credentials
- A Idira Identity service user and token
...
|------|-------------|------|---------|----------|
| `idsec_service_user` | Idira Identity service user | `string` | -- | yes |
| `idsec_service_token` | Idira Identity service token | `string` | -- | yes |
...
| Name | Description |
|------|-------------|
| `org_onboarding_id` | Idira CCE organization onboarding ID |
